$(document).ready(function(){
    ...
    for(var g=0; g<3; g++){ // major square row
        ...
        for(var h=0; h<3; h++){ // major square col
            var sq_cell = $('<th scope="col" class="bg-transparent" style="border:5px solid transparent">');
            ...
            for(var i=1; i<4; i++){ // minor square row
                ...
                for(var j=1; j<4; j++){ // minor square col
                    ...
                    var field = $('<input id="'+[3*g+i, 3*h+j].toString()+'"style="height:7vh; font-size: 4vh; border-radius: 0" class="form-control text-center" maxlength="1" oninput="this.value=this.value.replace(/[^1-9]/g,'+'\'\''+');">');
                    cell.append(field);
                    row.append(cell);
                }
